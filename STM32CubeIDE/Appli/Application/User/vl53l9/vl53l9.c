...
// NOTE: Modification of the pre-defined settings is not covered by ST Open Office usage under safety recommendations
...
// TODO: MISRAC2012-Rule-12.2: RHS argument is out of range of the shift operator
...
#define FRAME_SIZE_BINNING_4  (24 * 24) // cropped: 24 * 20
#define FRAME_SIZE_BINNING_6  (18 * 14)
#define FRAME_SIZE_BINNING_8  (12 * 10)
#define FRAME_SIZE_BINNING_12 (8 * 8) // cropped: 8 * 6
...
#define RAW_BUFFER_SIZE(binning) (FRAME_SIZE(binning) * 3 * 2 + DSS_SIZE(binning) + VL53L9_STATUS_SIZE)
...
int vl53l9_set_hw_config(void *const p_dev, vl53l9_hw_config_t config) {
    ...
    ret = vl53l9_write8(p_dev, VL53L9_REGADDR_OUTPUT_IF, config.output_interface);
    CHECK_RET(ret);
    ret = vl53l9_write8(p_dev, VL53L9_REGADDR_FRAME_SIGNALING_MODE, config.signaling_mode);
    CHECK_RET(ret);
    ret = vl53l9_write8(p_dev, VL53L9_REGADDR_INTR_OUTPUT_MODE, config.interrupt_pad_mode);
    CHECK_RET(ret);
    ...
    CHECK_RET(ret);
    status_line_cfg = (uint16_t)((config.csi_status_line_datatype & (uint8_t)0x2f) |
...
int vl53l9_get_context_selection(void *const p_dev, vl53l9_context_t *p_context) {
    int ret;
    uint8_t data;
...
int vl53l9_set_context_selection(void *const p_dev, vl53l9_context_t context) {
    int ret;
    ...
    uint16_t cab_short_dist_scale = (context == VL53L9_CONTEXT_SHORT) ? 256 : 683;
    uint16_t cab_long_dist_scale = 2048;

    uint32_t cab_dist_scale = 0;
...
int vl53l9_get_context_binning(void *const p_dev, vl53l9_context_t context, uint8_t *p_binning) {
    CHECK_NULL_PTR(p_dev);
    CHECK_NULL_PTR(p_binning);
    return vl53l9_read8(p_dev, VL53L9_REGADDR_STANDBY_BINNING((uint16_t)context), p_binning);
}

int vl53l9_set_context_binning(void *const p_dev, vl53l9_context_t context, uint8_t binning) {
    int ret;
...
int vl53l9_get_context_exposure(void *const p_dev, vl53l9_context_t context, uint16_t *p_exposure_ms) {
    ...
    // TODO: investigate why this isn't producing the same result
    // ret =
    //     vl53l9_read(p_dev, VL53L9_REGADDR_STREAM_NB_SHOT_STEP(1U, (uint16_t)context), (uint8_t *)shots,
    //     sizeof(shots));
    // CHECK_RET(ret);

    for (uint16_t i = 0; i < 7; i++) {
        ret = vl53l9_read32(p_dev, VL53L9_REGADDR_STREAM_NB_SHOT_STEP(i + 1, (uint16_t)context), &shots[i]);
        CHECK_RET(ret);
    }

    for (uint16_t i = 0; i < 7; i++) {
        expo_acc += (2.0f * (64 + blanking[i]) * (float)shots[i] / 1000000.0f);
    }
...
int vl53l9_set_context_exposure(void *const p_dev, vl53l9_context_t context, uint16_t exposure_ms) {
    ...
    for (uint8_t i = 0U; i < 7U; i++) {
        blank_sum += (64U + blanking[i]) * ratios[i];
    }
...
int vl53l9_get_frame(void *const p_dev, uint8_t *p_buffer, uint16_t size) {
    ...
    // return error if the device is configured to output data through i3c interface
    ret = vl53l9_read8(p_dev, VL53L9_REGADDR_OUTPUT_IF, &data);
    CHECK_RET(ret);
    if (data != VL53L9_OUTPUT_I3C) {
        return VL53L9_ERROR_INVALID_OPERATION;
    }
...
//    delay_ms_approx(20);
    ...
    CHECK_RET(ret);
    context = (vl53l9_context_t)_FIELD_GET(VL53L9_REGFIELD_CONTEXT_SELECTION, context);
    uint8_t binning;
    ...
    uint16_t buffer_size = resolution * 2U;
    uint16_t dss_buffer_size = (uint16_t)(resolution / 2U);
    uint16_t offset = 0U;

    // read depth, amplitude and ambient
    ret = vl53l9_read(p_dev, VL53L9_REGADDR_FB_DEPTH, &p_buffer[offset], 3 * buffer_size);
    CHECK_RET(ret);
    offset = 3U * buffer_size;
    ...
    CHECK_RET(ret);
    ret = vl53l9_read(p_dev, VL53L9_REGADDR_FB_DEPTH, &p_buffer[offset], dss_buffer_size);
    CHECK_RET(ret);
    ret = _write_cmd(p_dev, COMMAND_DSS_LUT_UNMAP, 5);
    CHECK_RET(ret);
    offset += dss_buffer_size;
...
int vl53l9_get_frame_async(void *const p_dev, uint8_t *p_buffer, uint16_t size) {
    ...
    // read depth, amplitude and ambient
    ret = vl53l9_read_async(p_dev, VL53L9_REGADDR_FB_DEPTH, &p_buffer[offset], 3 * buffer_size);
    CHECK_RET(ret);
...
int vl53l9_get_frame_async_ack(void *const p_dev, uint8_t *p_buffer, uint16_t size) {
    ...
    uint16_t buffer_size = resolution * 2U;
    uint16_t dss_buffer_size = (uint16_t)(resolution / 2U);
    uint16_t offset = 3U * buffer_size;
...
static int _init_default_config(void *const p_dev) {
    ...
    ret = vl53l9_write16(p_dev, VL53L9_REGADDR_STREAM_SWITCHOVER_DIST(context), 325);
    CHECK_RET(ret);
    ...
    ret = vl53l9_write8(p_dev, VL53L9_REGADDR_STREAM_RTN_SHORT_OFFSET(context), 2);
    CHECK_RET(ret);
    ret = vl53l9_write16(p_dev, VL53L9_REGADDR_STREAM_SWITCHOVER_DIST(context), 650);
    CHECK_RET(ret);
    ...
    // common
    ret = vl53l9_write8(p_dev, VL53L9_REGADDR_SYNCHRO, (uint8_t)VL53L9_SYNC_AUTONOMOUS);
    CHECK_RET(ret);
    ret = vl53l9_write8(p_dev, VL53L9_REGADDR_FORMAT, (uint8_t)FORMAT_WIDE);
    CHECK_RET(ret);
    ...
    return vl53l9_read32(p_dev, 0xd524, &data);
}
...
static int _is_valid_csi_config(void *const p_dev) {
    ...
    if (csi_height * csi_width < frame_size_expected) {
        return VL53L9_ERROR_INVALID_STATE;
    }
    return ret;
}
...
static unsigned int _get_raw_frame_resolution(uint8_t binning) {
    ...
    switch (binning) {
    case 2:
        return FRAME_SIZE_BINNING_2;
    case 4:
        return FRAME_SIZE_BINNING_4;
    case 6:
        return FRAME_SIZE_BINNING_6;
    case 8:
        return FRAME_SIZE_BINNING_8;
    case 12:
        return FRAME_SIZE_BINNING_12;
    case 24:
        return FRAME_SIZE_BINNING_24;
    default:
        return 0;
    }
}
